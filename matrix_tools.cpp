...
Matrix createMatrix(int rows, int cols)
{
    ...
    // Crear espacio para todos los elementos de la matriz
...
void fillRandomMatrix(Matrix* m, int minValue, int maxValue, unsigned int seed)
{
    ...
    // Iniciar los números aleatorios
    srand(seed);

    // Llenar todos los espacios de la matriz
...
void zeroMatrix(Matrix* m)
{
    ...
    // Poner todos los elementos en cero
    for (int i = 0; i < m->rows * m->cols; i++)
    {
        m->data[i] = 0;
    }
...
void printMatrixSample(const Matrix* m, int maxRows, int maxCols, const char* name)
{
    ...
    // Imprimir solamente las filas y columnas solicitadas
    for (int i = 0; i < rLim; i++)
    {
        for (int j = 0; j < cLim; j++)
        {
            printf("%5d ", m->data[i * m->cols + j]);
        }
...
void transposeMatrix(const Matrix* src, Matrix* dst)
{
    ...
    for (int i = 0; i < src->rows; i++)
    {
        ...
        {
            dst->data[j * dst->cols + i] =
                src->data[i * src->cols + j];
        }
    }
}
